    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res =  await SELECT.from ("sap.ariba.Event_AN").where(
                    {
                 ...
                 if(res.length==0){
                     //New record, insert
                    await INSERT .into ("sap.ariba.Event_AN") .entries (oDataCleansed) ;

                 }else{
                     //Update existing record
                    await UPDATE ("sap.ariba.Event_AN") .set (oDataCleansed) .where(
                        {
                            Realm : oDataCleansed.Realm ,
                            EventId : oDataCleansed.EventId,
                            ItemId : oDataCleansed.ItemId

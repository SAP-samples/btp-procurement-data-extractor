    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res =  await SELECT.from ("sap.ariba.InvoiceExceptions_AN").where(
                    { 
                 ...
                 if(res.length==0){
                     //New record, insert
                    await INSERT .into ("sap.ariba.InvoiceExceptions_AN") .entries (oDataCleansed) ;
                                  
                 }else{
                     //Update existing record
                    await UPDATE ("sap.ariba.InvoiceExceptions_AN") .set (oDataCleansed) .where(
                        { 
                            Realm : oDataCleansed.Realm ,
                            InvoiceId : oDataCleansed.InvoiceId,
                            InvoiceLineNumber : oDataCleansed.InvoiceLineNumber,

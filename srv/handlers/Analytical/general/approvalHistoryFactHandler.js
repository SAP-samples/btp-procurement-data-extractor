    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res = await SELECT.from ("sap.ariba.ApprovalHistory_AN").where(
                    {
                ...
                if(res.length==0){
                     //New record, insert
                    await INSERT .into ("sap.ariba.ApprovalHistory_AN") .entries (oDataCleansed) ;

                } else {
                    //Update existing record
                    await UPDATE ("sap.ariba.ApprovalHistory_AN") .set (oDataCleansed) .where(
                        {
                            Realm : oDataCleansed.Realm ,
                            ApprovableId : oDataCleansed.ApprovableId,
                            ApprovableType : oDataCleansed.ApprovableType,

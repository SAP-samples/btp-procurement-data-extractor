    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res =  await SELECT.from ("sap.ariba.ContractLineItems_AN").where(
                    { 
                 ...
                 if(res.length==0){
                     //New record, insert
                    await INSERT .into ("sap.ariba.ContractLineItems_AN") .entries (oDataCleansed) ;
                                  
                 }else{
                     //Update existing record
                    await UPDATE ("sap.ariba.ContractLineItems_AN") .set (oDataCleansed) .where(
                        { 
                            Realm : oDataCleansed.Realm ,
                            SplitAccountingNumber : oDataCleansed.SplitAccountingNumber ,
                            ContractLineNumber : oDataCleansed.ContractLineNumber ,

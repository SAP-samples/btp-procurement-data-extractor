    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res =  await srv.run ( SELECT.from ("sap.ariba.ContractsDim").where(
                    { 
                 ...
                 if(res.length==0){
                     //New record, insert
                    await srv.run( INSERT .into ("sap.ariba.ContractsDim") .entries (oDataCleansed) );
                                  
                 }else{
                     //Update existing record
                    await srv.run ( UPDATE ("sap.ariba.ContractsDim") .set (oDataCleansed) .where(
                        { 
                            Realm : oDataCleansed.Realm ,
                            ContractId : oDataCleansed.ContractId,                        
                            SourceSystem : oDataCleansed.SourceSystem} )

    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res =  await srv.run ( SELECT.from ("sap.ariba.ProjectInfo").where(
                    {
                ...
                if(res.length==0){
                     //New record, insert
                    await srv.run( INSERT .into ("sap.ariba.ProjectInfo") .entries (oDataCleansed) );

                } else {
                    //Update existing record
                    await srv.run ( UPDATE ("sap.ariba.ProjectInfo") .set (oDataCleansed) .where(
                        {
                            Realm : oDataCleansed.Realm ,
                            ProjectId : oDataCleansed.ProjectId
                        })
